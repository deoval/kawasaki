...
if ((int)$m >0 && (int)$m<13 && (int)$q==1)
	$db->setWhere(" and s.id_cliente=". $id_cliente . " and s.ativo=1 and date_format(data,'%m')=" . $m . " and date_format(data,'%d') <= 15");
else if ((int)$m >0 && (int)$m<13 && (int)$q==2)
	$db->setWhere(" and s.id_cliente=". $id_cliente . " and s.ativo=1 and date_format(data,'%m')=" . $m. " and date_format(data,'%d') >= 15");
else	
$db->setWhere(" and s.id_cliente=". $id_cliente . " and s.ativo=1");

$db->Query($db->Select());


/*$solicitacoes = Solicitacao::join('cliente as c', 's.id_cliente', '=', 'c.id_cliente')
->join( 'motoboy as m', 's.id_motoboy', '=', 'm.id_motoboy')
...
->select('s.id_solicitacao as solicitacao_id', 's.data', 's.ativo', 's.valor', 'c.nome as cliente', 's.id_cliente as cliente_id',
'm.nome as motoboy', 'm.id_motoboy as motoboy_id',
'seb.id_solicitacao_endereco as sendbusca_id', 'seb.endereco as enderecoa', 'seb.numero as numeroa', 'seb.bairro as bairroa', 'seb.cidade as cidadea',
'see.id_solicitacao_endereco as sendentrega_id' , 'see.endereco as enderecob' , 'see.numero as numerob', 'see.bairro as bairrob', 'see.cidade as cidadeb')
